...
NSString * Drupanium$ModuleRequireFormat = @"(function(exports){"
...
-(void)didStartNewContext:(KrollContext*)kroll
{
    ...
    // Load the "Drupanium" object into the global scope
...
{
	NSString *js = [[NSString alloc] initWithFormat:Drupanium$ModuleRequireFormat,code];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			@throw [NSException exceptionWithName:@"org.drupanium.kroll" reason:[NSString stringWithFormat:@"Module \"%@\" failed to leave a valid exports object",path] userInfo:nil];
		}
